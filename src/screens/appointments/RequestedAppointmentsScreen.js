...
import RequestedAppointmentCard from "../../components/RequestedAppointmentCard";
...
const RequestedAppointmentsScreen = ({ navigation }) => {
  ...
  const fetchData = async (isRefresh = false) => {
    ...
    try {
      const response = await apiService.appointments.getAppointmentRequests();

      // Extract appointments from the response
      const allAppointments = response.data?.allAppointments || [];

      // Filter for pending appointments (requested status)
      const pendingAppointments = allAppointments.filter(app => app.bookingStatus === "pending");
  ...
  return (
    ...
    >
      {isEmpty ? (
        <EmptyState message="No requested appointments" icon="calendar-clock" />
      ) : (
        <>
...
const styles = StyleSheet.create({
  ...
  skeletonCard: {
    ...
    elevation: 2,
    padding: 16,
    borderWidth: 1, // Added borderWidth for skeleton
  },
  skeletonHeader: {
    flexDirection: "row",
    alignItems: "center",
...
export default RequestedAppointmentsScreen;
