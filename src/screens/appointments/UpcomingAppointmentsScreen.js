...
const UpcomingAppointmentsScreen = ({ navigation }) => {
  ...
  const fetchData = async (isRefresh = false) => {
    ...
    try {
      const response = await apiService.appointments.getUpcomingAppointments();
      ...
      // Filter for booked appointments only
      const bookedAppointments = allAppointments.filter(app => app.bookingStatus === "booked");
  ...
  return (
    ...
    >
      {isEmpty ? (
        <EmptyState message="No upcoming appointments" icon="calendar-blank" />
      ) : (
        <>
